...
function getAverage(gradesArr) {
    ...
    let average = 0;
    for (let i = 0; i < gradesArr.length; i++) {
        if (gradesArr[i].value == "A")
            average += 4.0;
        else if (gradesArr[i].value == "B")
            average += 3.0;
        else if (gradesArr[i].value == "C")
            average += 2.0;
        else if (gradesArr[i].value == "D")
            average += 1.0;
        else
            average += 0.0;
    }
...
function requirementCheck(status, avg) {
    ...
    let undergraduateCongratzMsg = "Undergraduate! Congratulations, you qualify, Here are the instructions to apply:\n >>>LINK WOULD GO HERE<<<\nAverage: "
        + avg + " | status: " + status + "\nrequires=" + undergraduateMinimum;
    let graduateCongratzMsg = "Graduate! Congratulations, you qualify, Here are the instructions to apply:\n >>>LINK WOULD GO HERE<<<\nAverage: "
        + avg + " | status: " + status + "\nrequires=" + graduateMinimum;
    let failMsg = "Thank you for your interest, but you do not qualify because you are too dumb\nAverage: "
        + avg + " | status: " + status;

    if ((status == "undergrad") && (avg >= undergraduateMinimum))
        document.getElementById("average_result").innerHTML = undergraduateCongratzMsg;
    else if ((status == "grad") && (avg >= graduateMinimum))
        document.getElementById("average_result").innerHTML = graduateCongratzMsg;
    else
        document.getElementById("average_result").innerHTML = failMsg;
...
function processForm(status) {
    ...
    for (let i = 0; i < inputz.length; i++) {
        if (checkInput(inputz[i].value) === false) {
            ...
            document.getElementsByClassName("error_message_form")[0].style.display = "block";
            document.getElementsByClassName("error_message_form")[0].innerHTML = errorMessage;
            errorFlag = true;
            break;
        }
        else {
    ...
    if (errorFlag == false) {
        ...
        console.log(gradesArr + "<-htmlcollection log\nAmount of elements:\n" + gradesArr.length);
...
function show(student_status) {
    if (student_status == "undergraduate") {
        document.getElementById("undergraduate_courses").style.visibility = "visible";
        ...
        document.getElementById("undergraduate_courses").style.visibility = "hidden";
        document.getElementById("undergraduate_courses").style.display = "none";
    }
    else
        document.getElementById("courses_display2").innerHTML = "no status";
}
